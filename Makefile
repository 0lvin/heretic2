...
all: config client server game ref_gl1 ref_gl3 ref_gles3 ref_soft ref_vk ref_gl4 player effects xatrix rogue ctf
...
$(BINDIR)/yquake2.exe : CFLAGS += -DAVMEDIADECODE
$(BINDIR)/yquake2.exe : LDLIBS += -lavformat -lavcodec -lswscale -lswresample -lavutil
endif
...
ifeq ($(WITH_AVCODEC),yes)
$(BINDIR)/quake2 : CFLAGS += -DAVMEDIADECODE
$(BINDIR)/quake2 : LDLIBS += -lavformat -lavcodec -lswscale -lswresample -lavutil
endif
...
$(BINDIR)/ref_gl4.dll : LDFLAGS += -shared
...
$(BINDIR)/ref_gl4.dylib : LDFLAGS += -shared
...
$(BINDIR)/ref_gl4.so : CFLAGS += -fPIC
$(BINDIR)/ref_gl4.so : LDFLAGS += -shared

endif # OS specific ref_gl4 stuff

$(BUILDDIR)/ref_gl4/%.o: %.c
	@if [ -z $(QUIET) ]; then\
		echo "===> CC $<";\
	fi
	${Q}mkdir -p $(@D)
...
$(BINDIR)/baseq2/player.dll : LDFLAGS += -shared
...
player:
	...
	${Q}mkdir -p $(BINDIR)/baseq2
	$(MAKE) $(BINDIR)/baseq2/player.dylib
...
$(BINDIR)/baseq2/player.dylib : CFLAGS += -fPIC
$(BINDIR)/baseq2/player.dylib : LDFLAGS += -shared
...
$(BINDIR)/baseq2/player.so : CFLAGS += -fPIC -Wno-unused-result
$(BINDIR)/baseq2/player.so : LDFLAGS += -shared
endif
...
effects:
	...
	${Q}mkdir -p $(BINDIR)/baseq2
	$(MAKE) $(BINDIR)/baseq2/effects.dll

$(BUILDDIR)/effects/%.o: %.c
	@if [ -z $(QUIET) ]; then\
		echo "===> CC $<";\
	fi
	${Q}mkdir -p $(@D)
...
$(BINDIR)/baseq2/effects.dll : LDFLAGS += -shared
...
effects:
	...
	${Q}mkdir -p $(BINDIR)/baseq2
	$(MAKE) $(BINDIR)/baseq2/effects.dylib
...
$(BINDIR)/baseq2/effects.dylib : CFLAGS += -fPIC
$(BINDIR)/baseq2/effects.dylib : LDFLAGS += -shared
...
$(BINDIR)/baseq2/effects.so : CFLAGS += -fPIC -Wno-unused-result
$(BINDIR)/baseq2/effects.so : LDFLAGS += -shared
endif
...
# $(BINDIR)/baseq2/player.so
ifeq ($(YQ2_OSTYPE), Windows)
$(BINDIR)/baseq2/player.dll : $(PLAYER_OBJS)
	@echo "===> LD $@"
	${Q}$(CC) $(LDFLAGS) $(PLAYER_OBJS) $(LDLIBS) -o $@
	$(Q)strip $@
else ifeq ($(YQ2_OSTYPE), Darwin)
$(BINDIR)/baseq2/player.dylib : $(PLAYER_OBJS)
	@echo "===> LD $@"
	${Q}$(CC) $(LDFLAGS) $(PLAYER_OBJS) $(LDLIBS) -o $@
else
$(BINDIR)/baseq2/player.so : $(PLAYER_OBJS)
	@echo "===> LD $@"
	${Q}$(CC) $(LDFLAGS) $(PLAYER_OBJS) $(LDLIBS) -o $@
endif

# $(BINDIR)/baseq2/effects.so
ifeq ($(YQ2_OSTYPE), Windows)
$(BINDIR)/baseq2/effects.dll : $(EFFECTS_OBJS)
	@echo "===> LD $@"
	${Q}$(CC) $(LDFLAGS) $(EFFECTS_OBJS) $(LDLIBS) -o $@
	$(Q)strip $@
else ifeq ($(YQ2_OSTYPE), Darwin)
$(BINDIR)/baseq2/effects.dylib : $(EFFECTS_OBJS)
	@echo "===> LD $@"
	${Q}$(CC) $(LDFLAGS) $(EFFECTS_OBJS) $(LDLIBS) -o $@
else
$(BINDIR)/baseq2/effects.so : $(EFFECTS_OBJS)
	@echo "===> LD $@"
	${Q}$(CC) $(LDFLAGS) $(EFFECTS_OBJS) $(LDLIBS) -o $@
endif
