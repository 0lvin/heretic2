...
static qboolean
FXMorkMissileTrailThink(struct client_entity_s *self, centity_t *owner)
{
	float scale;
...
static qboolean
FXMorkPPTrailThink(struct client_entity_s *self, centity_t *owner)
{
	...
	TrailEnt->r.tile = 1;
	scale = 1.5 * AVG_VEC3T(self->r.scale) / 0.35;
	VectorSet(TrailEnt->r.scale, scale, scale, scale);
	TrailEnt->alpha = 1.0;
	scale = AVG_VEC3T(self->r.scale) / 0.35;
	VectorSet(TrailEnt->r.scale, scale, scale, scale);
...
static qboolean
FXMPPExplosionBallThink(client_entity_t *explosion, centity_t *owner)
{
	explosion->LifeTime--;
...
FXMPPExplosionSmallBallThink(client_entity_t *explosion, centity_t *owner)
{
	...
	{	// The explosion ball becomes visible and flashes
		...
		flash->r.flags |= RF_FLARE | RF_TRANS_ADD_ALPHA | RF_TRANSLUCENT;// | RF_FULLBRIGHT;
		flash->r.frame = 1;
		flash->radius = 128;
...
//	newcore->Update = FXMPPExplosionCoreUpdate;
	...
	newcore->r.frame = 0;//1
	newcore->r.flags |= RF_FLARE | RF_TRANS_ADD_ALPHA | RF_TRANSLUCENT;// | RF_FULLBRIGHT;
	newcore->radius	= 128;
...
//	explosion = ClientEntity_new(type, flags, origin, NULL, 17);
	...
	explosion->alpha = 1.0;
	VectorSet(explosion->r.scale, 2, 2, 2);//0.1;
...
/*===============================
  ...
  ===============================*/
static qboolean
FXMorkPPReadyThink(struct client_entity_s *self, centity_t *owner)
{
	int		add_rate = 5;
	client_particle_t	*spark;
	int		i, numparts;
...
static qboolean
FXMorkShieldThink(struct client_entity_s *self, centity_t *owner)
{
	return true;
}

static qboolean
FXMorkShieldForm(struct client_entity_s *self, centity_t *owner)
{
	client_entity_t	*Trail;

	if (AVG_VEC3T(self->r.scale) <= 2.5)
	{
		return false;
		self->d_alpha = 0.0;
		self->d_scale = 0.0;
	...
	if (AVG_VEC3T(self->r.scale) <= 3.0 && !self->SpawnInfo)
	{
		self->SpawnInfo = 1;
		...
		VectorCopy(self->r.scale, Trail->r.scale);
...
client_entity_t *MorkMakeLightningPiece(vec3_t start, vec3_t end, float radius, int lifetime, qboolean plasma)
{
	...
	lightning->r.flags |= RF_FLARE | RF_TRANS_ADD_ALPHA;
...
/*
	if(flags & CEF_FLAG6)
	{
		...
		VectorSet(ring->r.scale, 1.0f, 1.0f, 1.0f);
		ring->alpha = 0.5;
		ring->SpawnInfo = 0;
...
static qboolean
FXMorkEyes2(struct client_entity_s *self, centity_t *owner)
{
	vec3_t	angles, forward, right, pos, pos2, side;
	int		num_parts, e, n;
	client_particle_t	*flame;
...
//		flame->extraUpdate = MorkEyesParticleUpdate;
	...
	fx->r.flags |= RF_TRANSLUCENT | RF_FLARE | RF_TRANS_ADD_ALPHA;
...
static qboolean
ShoveInnerThink (struct client_entity_s *self, centity_t *owner)
{
	if (self->alpha <= 0.1 || self->alpha > 2.0 || AVG_VEC3T(self->r.scale) <= 0.1)
		return false;
...
static qboolean
ShoveOuterThink (struct client_entity_s *self, centity_t *owner)
{
...
static qboolean
FXMorkShove(struct client_entity_s *self, centity_t *owner)
{
	client_entity_t	*TrailEnt;
...
static qboolean
DreamyHyperMechaAtomicGalaxyPhaseIIPlusEXAlphaSolidProRad_ParticleUpdate_opt (struct client_particle_s *self, int crap)
{
	float	d_time, d_time2;
	int		d_msec, yaw;
...
//trail
	...
	TrailEnt->r.flags |= RF_TRANSLUCENT | RF_TRANS_ADD_ALPHA | RF_FLARE;
	TrailEnt->r.model = Morkproj_models[2];
...
void FXCWStars (centity_t *owner,int type,int flags, vec3_t vel)
{
	...
	fx->r.color.g = 50;
	fx->r.color.b = 255;
	fx->r.flags |= RF_FULLBRIGHT | RF_TRANSLUCENT | RF_FLARE | RF_TRANS_ADD_ALPHA;
...
void FXMorkMissile ( centity_t *owner, vec3_t startpos )
{
	...
	while (i--)
	{
		...
		fx->r.spriteType = SPRITE_LINE;
		fx->r.flags |= RF_FLARE | RF_TRANSLUCENT | RF_FULLBRIGHT;
...
void FXMorkMissileHit ( vec3_t origin, vec3_t dir )
{
	...
	fx->r.model = morc_models[3];
	fx->r.flags |= RF_FLARE | RF_TRANS_ADD_ALPHA | RF_TRANSLUCENT | RF_FULLBRIGHT | RF_NODEPTHTEST;
...
void FXMorkTrackingMissile ( centity_t *owner, vec3_t origin, vec3_t velocity )
{
	...
	Trail->r.flags |= RF_FULLBRIGHT | RF_TRANSLUCENT | RF_FLARE | RF_TRANS_ADD_ALPHA;
...
qboolean mssithra_explosion_think (client_entity_t *self, centity_t *owner)
{
	...
	explosion->r.flags |= RF_FULLBRIGHT | RF_FLARE | RF_TRANS_ADD_ALPHA;
	VectorSet(explosion->r.scale, 0.1, 0.1, 0.1);
