								...
								vec3_t DefaultStartPos,vec3_t ActualStartPos, edict_t *caster)
{
	...
	LowerbackJointAngles[PITCH]=GetJointAngle(caster->rrs.rootJoint+CORVUS_LOWERBACK,PITCH);
	LowerbackJointAngles[YAW]=GetJointAngle(caster->rrs.rootJoint+CORVUS_LOWERBACK,YAW);
	LowerbackJointAngles[ROLL]=GetJointAngle(caster->rrs.rootJoint+CORVUS_LOWERBACK,ROLL);
	Matrix3FromAngles(LowerbackJointAngles,LowerRotationMatrix);

	UpperbackJointAngles[PITCH]=GetJointAngle(caster->rrs.rootJoint+CORVUS_UPPERBACK,PITCH);
	UpperbackJointAngles[YAW]=GetJointAngle(caster->rrs.rootJoint+CORVUS_UPPERBACK,YAW);
	UpperbackJointAngles[ROLL]=GetJointAngle(caster->rrs.rootJoint+CORVUS_UPPERBACK,ROLL);
