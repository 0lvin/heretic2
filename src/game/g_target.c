...
use_target_changelevel(edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (deathmatch->value && !((int)dmflags->value & DF_ALLOW_EXIT) &&
		(other != world))
	{
		T_Damage(activator, self, self, vec3_origin, other->s.origin,
				vec3_origin, 10000, 10000, DAMAGE_AVOID_ARMOR,MOD_EXIT);
		return;
	}
...
use_target_splash(edict_t *self, edict_t *other /* unused */, edict_t *activator)
{
	...
	if (self->dmg)
	{
		T_DamageRadius(self, activator, NULL,
			self->dmg + 40, self->dmg, self->dmg / 4, DAMAGE_NORMAL,MOD_DIED);
	}
}
