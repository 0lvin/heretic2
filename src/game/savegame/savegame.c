	...
	switch (field->type)
	{
		...
		case F_ITEM:
			...
			*(gitem_t **)p = GetItemByIndex(index);
			break;
		case F_FUNCTION:
			*(const byte **)p = ReadFunction(f, *(int *)p, GetFunctionList(field->ofs, fpl));
			break;
		case F_MMOVE:
			*(const mmove_t **)p = ReadMmove(f, *(int *)p);
			break;
		default:
			fclose(f);
			gi.error("%s: unknown field type", __func__);
...
WriteLevelLocals(FILE *f)
{
	...
	for (i = 0; i< level.active_buoys; i++)
	{
		...
		{
			WriteField1 (f, field, (byte *)&temp.buoy_list[i], sd_level.fplist);
		}
	}
	...
	for (i = 0; i< level.active_buoys; i++)
	{
		...
		for (field = bouyfields; field->name; field++)
		{
			WriteField2 (f, field, (byte *)&level.buoy_list[i], sd_level.fplist);
		}
	}
}
...
ReadLevelLocals(FILE *f)
{
	...
	for (i = 0; i< level.active_buoys; i++)
	{
		...
		for (field=bouyfields ; field->name ; field++)
		{
			ReadField (f, field, (byte *)&level.buoy_list[i], sd_level.fplist);
		}
	}
