...
{
	ent->rrs.clientEffects.numEffects = 0;
}
...
SV_CreatePersistantEffect(edict_t* ent, int type, int flags, vec3_t origin, char* format, ...)
{
	...
	if (ent != NULL)
	{
		ent->rrs.clientEffects.buf = &effect->buf[0];
		ent->rrs.clientEffects.bufSize = sizeof(effect->buf);
		ent->rrs.clientEffects.numEffects = 1;

		if (type == (int)FX_MAGIC_PORTAL)
		{
			ent->rrs.clientEffects.isPersistant = true;
		}
		else
		{
			ent->rrs.clientEffects.isPersistant = false;
		}
	}
