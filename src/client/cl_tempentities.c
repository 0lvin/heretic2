...
CL_ParseParticles(void)
{
	...
	MSG_ReadPosExt(&net_message, pos, cls.serverProtocol);
...
CL_ParseBeam(struct model_s *model)
{
	...
	MSG_ReadPosExt(&net_message, start, cls.serverProtocol);
	MSG_ReadPosExt(&net_message, end, cls.serverProtocol);
...
CL_ParseBeam2(struct model_s *model)
{
	...
	MSG_ReadPosExt(&net_message, start, cls.serverProtocol);
	MSG_ReadPosExt(&net_message, end, cls.serverProtocol);
	MSG_ReadPosExt(&net_message, offset, cls.serverProtocol);
...
CL_ParseSteam(void)
{
	...
	if (id != -1) /* sustains */
	{
		...
		if (free_sustain)
		{
			...
			MSG_ReadPosExt(&net_message, s->org, cls.serverProtocol);
...
CL_ParseTEnt(void)
{
	...
	switch (type)
	{
		...
		case TE_BLUEHYPERBLASTER:
			MSG_ReadPosExt(&net_message, pos, cls.serverProtocol);
			MSG_ReadPosExt(&net_message, dir, cls.serverProtocol);
		...
		case TE_RAILTRAIL2:
			MSG_ReadPosExt(&net_message, pos, cls.serverProtocol);
			MSG_ReadPosExt(&net_message, pos2, cls.serverProtocol);
