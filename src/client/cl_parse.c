...
// jmarshall - Heretic 2 rotation fix from their ref_gl.dll
	if (bits & U_ANGLE1)
	{
		to->angles[0] = MSG_ReadAngle(&net_message) / ANGLE_1;
	}

	if (bits & U_ANGLE2)
	{
		to->angles[1] = MSG_ReadAngle(&net_message) / ANGLE_1;
	}

	if (bits & U_ANGLE3)
	{
		to->angles[2] = MSG_ReadAngle(&net_message) / ANGLE_1;
	}
// jmarshall end
...
CL_ParsePlayerstate(frame_t *oldframe, frame_t *newframe, int protocol)
{
	...
	for (i = 0; i < stats_size; i++)
	{
		if (statbits[(int)(i / 32)] & (1u << (i % 32)))
		{
			...
			{
				Com_DPrintf("Unsupported stats value %d: %d\n", i, MSG_ReadShort(&net_message));
			}
		}
	}
}
