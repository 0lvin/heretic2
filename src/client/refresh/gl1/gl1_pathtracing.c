...
{
	R_Printf(PRINT_ALL, "ERROR: The OpenGL features necessary for pathtracing are not available.\n");
...
AddAliasModel(entity_t *entity, model_t *model)
{
	...
	if ((entity->frame >= alias_header->num_frames) ||
		(entity->frame < 0))
	{
		R_Printf(PRINT_DEVELOPER, "Pathtracer: AddAliasModel %s: no such frame %d\n",
	...
	if ((entity->oldframe >= alias_header->num_frames) ||
		(entity->oldframe < 0))
	{
		R_Printf(PRINT_DEVELOPER, "Pathtracer: AddAliasModel %s: no such oldframe %d\n",
...
R_PreparePathtracer(void)
{
	...
	{
		R_Printf(PRINT_ALL, "R_PreparePathtracer: r_worldmodel is NULL!\n");
		return;
	}

	if (r_worldmodel->numnodes == 0)
	{
		R_Printf(PRINT_ALL, "R_PreparePathtracer: r_worldmodel->numnodes is zero!\n");
		return;
	}
	...
	R_Printf(PRINT_DEVELOPER, "R_PreparePathtracer: %d static trilights\n", pt_num_lights);
...
InitRandom(void)
{
	...
	R_Printf(PRINT_DEVELOPER, "InitRandom: Random number lookup texture has %d layers.\n", num_layers);
	...
	for (i = 0; i < num_layers; ++i)
	{
		...
		{
			R_Printf(PRINT_ALL, "InitRandom: Could not find blue noise texture image \"%s\".\n", image_name);
		}
	}
...
PrintObjectInfoLog(GLhandleARB object)
{
	...
	if (info_log_length > 0)
	{
		...
		R_Printf(PRINT_ALL, info_log_buffer);
...
CreateShaderPrograms(void)
{
	...
	if (status != GL_TRUE)
	{
		R_Printf(PRINT_ALL, "R_InitPathtracing: Vertex shader failed to compile\n");
	...
	if (status != GL_TRUE)
	{
		R_Printf(PRINT_ALL, "R_InitPathtracing: Fragment shader failed to compile\n");
	...
	if (status != GL_TRUE)
	{
		R_Printf(PRINT_ALL, "R_InitPathtracing: Program failed to link\n");
	...
	R_Printf(PRINT_DEVELOPER, "pt_current_world_matrix_loc   = %d\n", pt_current_world_matrix_loc);
	R_Printf(PRINT_DEVELOPER, "pt_previous_world_matrix_loc  = %d\n", pt_previous_world_matrix_loc);
	R_Printf(PRINT_DEVELOPER, "pt_exposure_loc               = %d\n", pt_exposure_loc);
	R_Printf(PRINT_DEVELOPER, "pt_gamma_loc                  = %d\n", pt_gamma_loc);
	R_Printf(PRINT_DEVELOPER, "pt_bump_factor_loc            = %d\n", pt_bump_factor_loc);
	R_Printf(PRINT_DEVELOPER, "pt_shadow_ray_node_offset_loc = %d\n", pt_shadow_ray_node_offset_loc);
	R_Printf(PRINT_DEVELOPER, "pt_rand_tex_layer_loc         = %d\n", pt_rand_tex_layer_loc);
...
RecompileShaderPrograms(void)
{
	...
	R_Printf(PRINT_ALL, "Shaders recompiled.\n");
}

static void
PrintStaticInfo(void)
{
	R_Printf(PRINT_ALL, "%6d trilight references\n", pt_num_trilight_references);
	R_Printf(PRINT_ALL, "%6d clusters\n", pt_num_clusters);
	R_Printf(PRINT_ALL, "%6d used nonstatic lightstyles\n", pt_num_used_nonstatic_lightstyles);
	R_Printf(PRINT_ALL, "%6d static vertices\n", pt_dynamic_vertices_offset);
	R_Printf(PRINT_ALL, "%6d static triangles\n", pt_dynamic_triangles_offset);
	R_Printf(PRINT_ALL, "%6d static entity lights\n", pt_dynamic_entitylights_offset);
	R_Printf(PRINT_ALL, "%6d static trilights\n", pt_dynamic_lights_offset);
}
